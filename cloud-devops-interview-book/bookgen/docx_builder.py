...
Infographics are reportlab drawings rasterised with PyMuPDF so mindmaps and
flows stay sharp in Word.
"""
...
class DocxBuilder:
    ...
    def _cover(self) -> None:
        ...
        self._para(
            ...
            "path, the evidence you would quote, the red flag that costs you the offer, and the "
            "follow-up the interviewer is already holding. Yellow highlights mark the load-bearing "
            "sentences for a fast revision pass.",
    ...
    def _contents(self) -> None:
        ...
        self._rule()
        self._para(
            f"{self.book.total} questions across {len(self.book.parts)} modules. Modules climb from "
            "Linux fundamentals to production incident scenarios. Inside each module, questions move "
            "from Foundation to Senior.")
    ...
    def _how_to_use(self) -> None:
        ...
        self._rule()
        self._para(
            "Most interview prep fails the same way: you recite a definition, the interviewer asks "
            "\"how would you prove that?\", and the conversation stops. Definitions are table stakes. "
            "What gets scored is whether you can name the owning layer, quote evidence, choose the "
            "smallest safe action, and say what you would change afterwards.")
        self._para(
            f"So every one of the {self.book.total} questions here is answered in six layers. Read the "
            "first layer if you are short on time. Read all six if this is a topic you would rather "
            "not be surprised by.")
        if self.highlight:
            ...
            self._write(
                p,
                "This is the quick-learning copy. In every question the opening claim of the "
            ...
            self._write(
                p,
                f". Read only the highlighted sentences for a fast revision pass through all "
                f"{self.book.total} questions, then come back to the full text where the highlight "
                "was not enough. A clean unhighlighted copy is published alongside this one.",
                10.5, INK_SOFT)
            _shade(p, "FBF3E2")
            _left_bar(p, "B7791F")
        ...
        self._para(
            "Each question is tagged Foundation, Intermediate or Senior. The tag is about how deep "
            "the expected answer goes on the same subject - not whether the topic is 'basic'.")
        self._image("answer_ladder")
        self.doc.add_page_break()
        self.doc.add_heading("Two frameworks that carry most of the interview", level=2)
        self._para(
            "Roughly a third of a senior platform interview is troubleshooting and another third is "
            "design. If you only memorise two things from this book, memorise these loops.")
        self._image("evidence_first_loop")
        self._para(
            "For design questions, swap the loop for: Requirement, Options, Trade-off, Decision, "
            "Risk, Verification. Say the requirement back before you propose anything.")
        self._image("scoring_card")
        self.doc.add_heading("How to revise quickly", level=2)
        self._para(
            "Day one: Linux through Kubernetes for breadth. Day two: AWS, Terraform, Python, Airflow. "
            "Day three: CI/CD, observability, security, and speak thirty production scenarios out loud "
            "with a timer. In the final hour, re-read only red flags and analogies.")
        self.doc.add_heading("Honest scope note", level=2)
        self._para(
            "Examples are illustrative. Product behaviour moves - spot-check version-sensitive details "
            "against current Linux, Kubernetes, AWS, Terraform, Airflow and security documentation "
            "before an interview. In the room, state the principle first, then ask which version the "
            "team runs.")
    ...
    def _question(self, q: Question) -> None:
        ...
        if q.qid:
            qid = head.add_run(f"    {q.qid}")
            qid.font.size = Pt(8)
            qid.font.color.rgb = RGBColor(0xFF, 0xFF, 0xFF)
        _spacing(head, before=10, after=2)

        title = self._para(q.q, size=12, color=INK, bold=True)
        _spacing(title, before=2, after=2)
        if q.topic:
            meta = self._para(f"Topic: {q.topic}", size=8.5, color=MUTED)
            _spacing(meta, before=0, after=4)
        ...
        if q.deep_dive:
            self._label("Extra context", SLATE)
            self._callout(q.deep_dive, "EDF1F4", "44576B", INK_SOFT, size=9.5)
        ...
        if q.strong_answer:
            self._label("What a strong answer adds", TEAL)
            self._para(q.strong_answer, lead=True, limit=1)
        ...
        if q.reference:
            self._para(f"Reference: {q.reference}", size=8, color=MUTED)
    ...
    def _part(self, part: Part) -> None:
        self.doc.add_page_break()
        p = self._para(
            f"PART {part.number}  ·  {part.span}  ·  {len(part.questions)} QUESTIONS",
        ...
        self._para(
            "Level mix - " + "  |  ".join(f"{k}: {v}" for k, v in levels.items()),
    ...
    def _closing(self) -> None:
        ...
        self._rule()
        self._para(
            "You will not learn anything new in the last hour, so use it to protect what you already "
            "know. Say these six sentences out loud until they sound unrehearsed:")
        for i, text in enumerate([
            "\"Let me start with impact and scope, then the evidence I would collect.\"",
            "\"The owning layer here is X, so that is where I would look first.\"",
            "\"The smallest safe action is Y, because it limits blast radius.\"",
            "\"I would validate against the customer-visible symptom, not just a green check.\"",
        ...
        self._para(
            "Before an interview, spot-check version-sensitive details against current Linux man pages, "
            "Kubernetes docs, AWS docs, Terraform docs, Airflow docs, and your organisation's security "
            "standards. If unsure in the room, say which release you are describing and offer to confirm.")
