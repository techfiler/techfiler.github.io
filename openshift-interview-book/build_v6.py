#!/usr/bin/env python3
"""Build the V6 supplement interview book as PDF and DOCX.

    python3 build_v6.py [--outdir output]
"""

from __future__ import annotations

import argparse
import os
import sys

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

from bookgen.docx_builder import DocxBuilder  # noqa: E402
from bookgen.pdf_builder import PdfBuilder  # noqa: E402
from content_supplement import build_book  # noqa: E402

BASENAME = "Platform_Engineer_Supplement_Interview_Book_V6_100QA"


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--outdir", default=os.path.join(os.path.dirname(
        os.path.abspath(__file__)), "output"))
    args = parser.parse_args()
    os.makedirs(args.outdir, exist_ok=True)

    book = build_book()
    print(f"parts: {len(book.parts)}   questions: {book.total}")
    if book.total != 100:
        print(f"ERROR: expected 100 questions, found {book.total}", file=sys.stderr)
        return 1

    editions = [
        (BASENAME, False),
        (f"{BASENAME}_Highlighted", True),
    ]

    for name, highlight in editions:
        pdf_path = os.path.join(args.outdir, f"{name}.pdf")
        docx_path = os.path.join(args.outdir, f"{name}.docx")

        PdfBuilder(book, pdf_path, highlight=highlight).build()
        print(f"wrote {pdf_path} ({os.path.getsize(pdf_path) / 1024:.0f} KB)")

        DocxBuilder(book, docx_path, highlight=highlight).build()
        print(f"wrote {docx_path} ({os.path.getsize(docx_path) / 1024:.0f} KB)")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
